...
namespace VideoPlaylist
{
    ...
    public class xxActorManager
    {
        ...
        public List<ActorData> FindMatchingEntries(List<string> tokens)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in result.Rows)
                {
                    ...
                    // Check for Forename-Surname pair
        ...
        public void PopulateChecklistBox(List<ActorData> actors, CheckedListBox checkListBox, int matchCount)
        {
            try
            {
                checkListBox.Items.Clear();
                //actors = FindMatchingEntries(tokens);
                if (actors.Count > 0)
                {
                    ...
                    {
                        if ( actor.MatchCount == matchCount )
        ...
        /// /// The ID of the inserted actor, or 0 if the operation fails. <summary>
        /// Inserts an actor with the provided full name, splitting into Forename and Surname, and sets Include to true. 
        /// 
        /// </summary>
        ...
        {
            if (string.IsNullOrWhiteSpace(fullName)) { DBLogger.LogError("InsertQuickActor failed: FullName cannot be empty."); return 0; }



            try
            {
                var parts = System.Text.RegularExpressions.Regex.Split(fullName.Trim(), @"[\s,.]+")
                  .Where(p => !string.IsNullOrEmpty(p)).ToArray();
                string forename = parts.Length > 0 ? parts[0] : "";
                string surname = parts.Length > 1 ? parts[parts.Length - 1] : "";

                fullName = fullName.ToMixedCase();
                forename = forename.ToMixedCase();
                surname = surname.ToMixedCase();
